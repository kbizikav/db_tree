...
#[derive(Clone, Debug)]
pub struct NodeElement<V: Leafable> {
    pub timestamp: u64,
...
pub trait NodeDB<V: Leafable + Serialize + DeserializeOwned>: std::fmt::Debug + Clone {
    async fn insert_node(&self, parent_hash: HashOut<V>, node: NodeElement<V>) -> NodeDBResult<()>;
    async fn get(&self, parent_hash: HashOut<V>) -> NodeDBResult<Option<NodeElement<V>>>;
...
pub struct MockNodeDB<V: Leafable> {
    nodes: Arc<RwLock<HashMap<HashOut<V>, NodeElement<V>>>>,
